...
Core invoice header:
  ...
  ${schemaType === 'construction' ? `grossAmount             — CONSTRUCTION: the "Work Completed This Period" TOTAL, and only that. Source: Continuation Sheet Column E "THIS PERIOD" — the total row, by priority: (1) GRAND TOTAL/GRAND TOTALS row in Col A, else (2) TOTAL/TOTALS, else (3) SUBTOTALS; use the LAST such occurrence. If a Sworn Statement page is present, use the "This Payment" column TOTAL row instead. NEVER source grossAmount from: Contract Price, Contract Amount, Total to Date, Amount Paid, Balance Due, Previous Payments, Current Payment Due, or Amount Certified — even if those are larger or more prominent. NEVER use an individual numbered line row — only the total/grand-total/subtotal row. 0.00 is valid. If no THIS PERIOD column exists, return NULL — never substitute another column.` : `grossAmount             — total amount due on the invoice (strip currency symbols)`}
...
5. AMOUNTS: strip currency symbols; use current-period/total-due, not cumulative.${schemaType === 'construction' ? `
   CONSTRUCTION grossAmount — the taxable base is the "Work Completed This Period" TOTAL, and only that. Source: Continuation Sheet Column E "THIS PERIOD" — the total row, by priority: (1) GRAND TOTAL/GRAND TOTALS row in Col A, else (2) TOTAL/TOTALS, else (3) SUBTOTALS; use the LAST such occurrence. If a Sworn Statement page is present, use the "This Payment" column TOTAL row instead. NEVER source grossAmount from: Contract Price, Contract Amount, Total to Date, Amount Paid, Balance Due, Previous Payments, Current Payment Due, or Amount Certified — even if those are larger or more prominent. NEVER use an individual numbered line row — only the total/grand-total/subtotal row. 0.00 is valid; if no THIS PERIOD column exists, return NULL — never substitute another column. If Doc AI's grossAmount value came from Contract Price or Total to Date, mark it CORRECTED.` : ``}
