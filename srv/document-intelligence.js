...
module.exports = class DocumentIntelligenceService extends cds.ApplicationService {
  ...
  async _handleProcessInvoice(req) {
    ...
    const claudeKeepItems = rawClaudeItems.filter(function(li){
      if (li.lineVerdict === 'CORRECTED' && parseFloat(li.amount) === 0) return false;
      return !li.isFreight && li.lineVerdict !== 'SUPPRESSED';
    });
    const claudeSuppressedItems = rawClaudeItems
      .filter(function(li){
        if (li.lineVerdict === 'CORRECTED' && parseFloat(li.amount) === 0) return true;
        return li.isFreight || li.lineVerdict === 'SUPPRESSED';
      })
...
- PRIOR-PERIOD / ZERO CURRENT-PERIOD: On multi-column invoices (fee schedules, progress billing, CBRE-style — Fee / Earned-to-Date / Previous Billing / Current Fee Billing), ONLY the current-period column is billable this invoice. If Doc AI extracted an amount from the wrong column (Earned-to-Date, Previous Billing, or any cumulative figure) for a line whose Current Fee Billing = $0.00: set amount=0 (the correct current-period billing is $0, NOT the earned-to-date figure). You may use lineVerdict="CORRECTED" or "SUPPRESSED". Set lineReason="Current Fee Billing $0.00 — Doc AI read prior-period/earned-to-date value ($[Doc-AI-extracted amount]); already billed in a prior period, not billable this invoice." Scope guard: ONLY when the invoice has a visible current-period column AND it reads $0 for this line. Does NOT apply to single-column invoices. Does NOT apply to lines with a non-zero current-period amount. Named-person/vendor summary lines (Rock Brook, Sheehan, Ivanoff) have real non-zero current-period amounts — leave them untouched.
...
- ${schemaType === 'construction' ? 'CONSTRUCTION: current-period column (Work Completed This Period > Current Bill > This Period); prefer Sworn Statement totals.' : 'NON-CONSTRUCTION: use the current-period / Current Fee Billing column for each line\'s amount. Return EVERY Doc AI line with an explicit verdict — do NOT silently omit any line. If the current-period column is $0.00 while a prior-period column (Earned-to-Date, Previous Billing, Cumulative, or similar) shows a non-zero value, set amount=0 (the current-period billing is $0) and explain in lineReason. Scope guard: ONLY when the invoice visibly has a current-period column and that column reads $0 for this row. Does NOT apply to single-column invoices; does NOT change amount for lines with a non-zero current-period value.'}
...
- PRIOR-PERIOD / ZERO CURRENT-PERIOD: On multi-column invoices (fee schedules, progress billing, CBRE-style — Fee / Earned-to-Date / Previous Billing / Current Fee Billing), ONLY the current-period column is billable this invoice. If Doc AI extracted an amount from the wrong column (Earned-to-Date, Previous Billing, or any cumulative figure) for a line whose Current Fee Billing = $0.00: set amount=0 (the correct current-period billing is $0, NOT the earned-to-date figure Doc AI captured). You may use lineVerdict="CORRECTED" or "SUPPRESSED" — the critical requirement is amount=0. Set lineReason="Current Fee Billing $0.00 — Doc AI read prior-period/earned-to-date value ($[Doc-AI-extracted amount]); already billed in a prior period, not billable this invoice." Scope guard: ONLY when the invoice has a visible current-period column AND it reads $0 for this line. Does NOT apply to single-column invoices. Does NOT apply to lines with a non-zero current-period amount. Named-person/vendor summary lines (Rock Brook, Sheehan, Ivanoff) have real non-zero current-period amounts — leave them untouched.
